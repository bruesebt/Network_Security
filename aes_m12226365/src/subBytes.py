#This implements the SubBytes operation in the AES encryption. 

#This s-box implementation is taken from https://medium.com/wearesinch/building-aes-128-from-the-ground-up-with-python-8122af44ebf9
...
# Uses the aes_sbox rows and columns to locate the correct byte
def lookup(val):
    byte = int(val, 16) #ensures that val is a byte and not a string
    x = byte >> 4
    y = byte & 15
    return aes_sbox[x][y]

# Description:
# subBytes takes a matrix as an input, and for every value in that matrix, uses 'aes_sbox' to lookup each value. First, the function
# creates a list of all the values. The values are converted to columns, and then the function overwrites the old matrix with 
# the new columns. 
def subBytes(matrix):
    values = []
    for i in matrix:
        for j in i:
            val = lookup(j)
            values.append("{0:02x}".format(val, "x"))
